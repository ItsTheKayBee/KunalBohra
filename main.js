...
function cam() {
    ...
    camera.position.set(0, 1100, 100);
...
function createWheel() {
    ...
    var shape = new Ammo.btCylinderShape(1.25, 1.25, 1.25);
...
function addRover() {
    ...
    //create rigid body
    var mass = 27000;
    var pos = { x: 0, y: marsRadius + 2, z: 0 };
    ...
    var rover = new Ammo.btRaycastVehicle(tuning, body, rayCaster);
    ...
    function addWheel(isFront, pos, radius, index) {
        ...
        wheelMeshes[index] = sph = createWheel();
        sph.position.x += pos.x;
        sph.position.y -= pos.y;
        sph.position.z += pos.z;
        sph.rotation.x = Math.PI / 2;
        pos = { x: sph.position.x + pos.x, y: sph.position.y - pos.y, z: sph.position.z + pos.z };
        quat = { x: 0, y: 0, z: 0, w: 1 };
        wheel = createModels(sph, shape, pos, quat, 0);
        chassis.add(wheel);
    }

    addWheel(true, new Ammo.btVector3(10, 3, 10), 2.5, FRONT_LEFT);
    addWheel(true, new Ammo.btVector3(-10, 3, -10), 2.5, FRONT_RIGHT);
    addWheel(false, new Ammo.btVector3(0, 3, 10), 2.5, MIDDLE_LEFT);
    addWheel(false, new Ammo.btVector3(0, 3, -10), 2.5, MIDDLE_RIGHT);
    addWheel(false, new Ammo.btVector3(-10, 3, 10), 2.5, BACK_LEFT);
    addWheel(false, new Ammo.btVector3(-10, 3, -10), 2.5, BACK_RIGHT);
...
function updatePhysics(deltaTime) {
    ...
    for (let i = 0; i < dynamicObjects.length; i++) {
        ...
        if (ms) {
            ...
            objThree.rotation.set(quaternion.x(), quaternion.y(), quaternion.z(), quaternion.w());
        }
    }
}